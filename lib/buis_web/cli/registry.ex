defmodule BuisWeb.Cli.Registry do
  @moduledoc """
  Découverte, au runtime, des domaines / resources / actions Ash configurés,
  et résolution module <-> slug pour les routes de la console CLI.

  Aucune resource n'est référencée en dur : tout vient de la config
  `:ash_domains` et de l'introspection Ash.
  """

  @doc "Domaines Ash déclarés dans la config applicative."
  def domains, do: Application.get_env(:buis, :ash_domains, [])

  @doc "Toutes les resources exposées par les domaines."
  def resources do
    domains()
  ...
  @doc "Slug URL d'une resource (ex: Buis.Bank.Account -> \"account\")."
  def resource_slug(resource), do: resource |> resource_label() |> Macro.underscore()

  @doc "Resource correspondant à un slug, ou nil."
  def resource_by_slug(slug), do: Enum.find(resources(), &(resource_slug(&1) == slug))
